...
###########################################################################################
    ...
    def populate_dict_from_list(self):
        # Update the testcaseResult dictionary 
        for item in self.testcases:
            self.testcaseResult[item]="Not tested"
    ...
    # API to schedule by test case;testcaseList should be a list;or use default one so we schedule whole campaign
        ...
        wlsPath=supportfunction.windows_to_wsl_path(campaignPath)

        Is_valid_xml, checkResult=supportfunction.is_valid_xml_file(wlsPath)
        if Is_valid_xml:
            pass
        else:
            print(checkResult)
        ...
        self.testcaseIndex=supportfunction.extract_npi_strings_from_file(campaignPath)
        ...
        if listCheckResult=="List of strings": # Schedule per Name 
            ...
            for item in testcaselist:
                self.testcaseResult[supportfunction.find_key_by_value(self.testcaseIndex,item)]="Not tested"
            if not len(self.testcaseResult.keys)==len(testcaselist):
                return self.error_code,"Input test case is not matching campaign XML test cases!"
    ...
    #API to run campaign and wait till the end 
        ...
        try:
            ...
            if response.status_code!=202:
                ...
                pattern_campaign=r'AUTOMATION_RUN Complete'
                pattern_npi=r'"(npi_[^"]+)"'
                while not supportfunction.find_message(result_text,pattern_campaign):
                    if result_text[-1]!="%":
                        npi_result=supportfunction.find_message(result_text,pattern_npi)
                        if npi_result:
                            match=re.search(pattern_npi,result_text)
                            captured_npi=match.group(1)
