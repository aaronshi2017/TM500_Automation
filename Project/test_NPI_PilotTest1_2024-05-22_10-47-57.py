...
def test_step1_command(moshell_command):
    result=moshell_command.command_execution("bl TLAB07X7F1D;setm TLAB07X7F1D dlChannelBandwidth 10000 ulChannelBandwidth 10000;deb TLAB07X7F1D")
    print(result)
...
def test_step6_schedule_campaign(TMA_API):
    path=r"D:/rApps/datasets/TM500/06_NPI_Projects/2024_01_NPI_PilotTest/01_tmtestcampaigns/tm_session_03.xml"
    testcase=[0]
...
def test_step9_update_report_to_database(TMA_API):
    if finalverdict=="PASS":
        result1, result2 = TMA_API.send_to_Database("NPI_PilotTest1",sessionName,False)
        print(result1,result2)
        assert result1 == 200
    else:
        print(finalverdict)
