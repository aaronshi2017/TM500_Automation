...
from Backup.class_moshellWSL import class_moshellcommandWSL
...
def test_step1_command(moshell_command):
    result=moshell_command.command_execution("st cell;alt;")
    print(result)
...
def test_step6_schedule_campaign(TMA_API):
    path=r"C:\Users\rante\Documents\VIAVI\TM500\5G NR\Test Mobile Application\NLA7.4.3 Rev2\MyCampaigns\NPI_TC-01.xml"
    testcase=['npi_01_registration_LTEFDD', 'npi_15_IntraHO_LTEFDD']
...
def test_step9_update_report_to_database(TMA_API):
    if finalverdict=="PASS":
        result1, result2 = TMA_API.send_to_Database("Test_5-18-test",sessionName,False)
        print(result1,result2)
        assert result1 == 200
    else:
        print(finalverdict)
